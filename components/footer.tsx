...
const SOCIALS = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.87a8.23 8.23 0 0 0 4.81 1.54V7a4.85 4.85 0 0 1-1.04-.31z" />
      </svg>
    ),
  },
  {
...
function FooterLink({ label, href }: { label: string; href: string }) {
  return (
    <Link
      href={href}
      className="group inline-flex text-white font-medium tracking-tight leading-tight text-lg lg:text-xl hover:text-[#b5f0c3] transition-colors duration-200"
    >
      <div className="relative overflow-hidden">
        <div className="transition-transform duration-300 group-hover:-translate-y-7">{label}</div>
        <div className="absolute top-0 left-0 translate-y-7 transition-transform duration-300 group-hover:translate-y-0">
          {label}
        </div>
      </div>
    </Link>
  ...
  return (
    ...
    >
      <div className="w-full px-0">
        <div className="p-2 grid relative">
          {/* Dark rounded bg */}
          <div className="absolute top-0 left-0 w-full h-full p-2 flex">
            <div className="w-full h-full bg-[#111111] rounded-3xl" />
          </div>
          ...
          <div className="col-start-1 row-start-1 grid grid-cols-12 pt-14 pb-6 relative z-20 lg:py-10 px-4 md:px-7 gap-x-3 md:gap-x-5 gap-y-3 md:gap-y-7">
            ...
            <div className="flex flex-col items-start justify-start col-span-12 mb-10 lg:mb-0 lg:col-span-4 gap-y-5">
              <h2 className="text-white text-2xl xl:text-3xl font-medium tracking-tight">
              ...
              <form className="w-full relative">
                <input
                  ...
                  className="appearance-none bg-white/10 rounded-full w-full text-white font-medium tracking-tight text-lg px-5 py-4 lg:text-xl lg:px-6 lg:py-5 placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/20"
                />
                <div className="absolute top-0 right-0 p-2">
                  <button
                    type="submit"
                    className="w-9 h-9 lg:w-12 lg:h-12 bg-[#b5f0c3] text-[#111111] rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 hover:bg-white hover:rotate-90 text-sm font-bold"
                  >
                    ↗
                  </button>
                </div>
              ...
              <div className="flex gap-1 flex-wrap">
                {SOCIALS.map((s) => (
                  <a
                    ...
                    className="inline-flex items-center gap-x-2 rounded-xl text-xs px-2 py-1 bg-white text-[#111111] hover:rounded-sm transition-all duration-200"
                    aria-label={s.label}
                  >
                    {s.icon}
            ...
            <div className="flex justify-between col-span-12 flex-wrap md:flex-row md:col-span-11 lg:col-span-6 lg:col-start-6 gap-y-10">
              {[NAV_COL_1, NAV_COL_2, NAV_COL_3].map((col, ci) => (
                <div
                  key={ci}
            ...
            <div className="col-span-12 flex justify-between flex-col mt-10 items-end md:flex-row lg:items-center lg:mt-0">
              <div className="flex gap-x-2 gap-y-1 flex-wrap items-center">
                {[
                  { text: "© 2025 Rise at Seven Ltd. All rights reserved" },
                  ...
                  <span key={i} className="inline-flex items-center gap-x-2">
                    {item.href ? (
                      <Link href={item.href} className="text-white/60 font-light text-xs hover:text-white transition-colors">
                        {item.text}
                      </Link>
                    ) : (
                      <span className="text-white/60 font-light text-xs">{item.text}</span>
                    )}
                    {i < 4 && <span className="w-1 h-1 rounded-full bg-white/40 inline-block" />}
                  </span>
                ))}
              </div>
              <div className="mt-1 md:ml-auto md:text-right lg:mt-0">
                <a href="https://madebyshape.co.uk" target="_blank" rel="noopener noreferrer" className="text-white/40 font-light text-xs hover:text-white/80 transition-colors">
                  Website MadeByShape
                </a>
              </div>
            </div>
